...
ALL_ATTENTION_FUNCTIONS = ["eager", "sdpa", "flash_attention_2"]
...
def scatter_mean(src, index, dim, dim_size):
    out = torch.zeros(src.shape[0], dim_size, src.shape[2], dtype=src.dtype, device=src.device)
    ones = torch.ones_like(src)
    count = torch.zeros(src.shape[0], dim_size, src.shape[2], dtype=src.dtype, device=src.device)
...
#     # num_groups = int(mapping_indices.max().item()) + 1
...
class CustomLlamaAttention(LlamaAttention):
    ...
    def forward(
        ...
        **kwargs: Unpack[FlashAttentionKwargs],
        ...
        K_m = repeat_kv(K_m, self.num_key_value_groups)
        V_m = repeat_kv(V_m, self.num_key_value_groups)
...
class LlavaLlamaForCausalLM(LlamaForCausalLM, LlavaMetaForCausalLM):
    ...
    def forward(
        ...
        # Custom forward logic to bypass LlamaForCausalLM restrictions on arguments
        output_attentions = output_attentions if output_attentions is not None else self.config.output_attentions
        output_hidden_states = (
            output_hidden_states if output_hidden_states is not None else self.config.output_hidden_states
        )
        ...
        outputs = self.model(
            ...
            # cache_position=cache_position, # older transformers might not pass this to model? LlavaLlamaModel has it
            cache_position=cache_position,
            # Extra args
            im_pos=im_pos,
            pos_transform=pos_transform,
