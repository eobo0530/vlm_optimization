        ...
        if self._tome_info["merge_mode"] == "instance_level":
            ...
            if not self.training and not self.update_threshold:
                if self.specified_threshold is not None:
                    ...
                    # If the stored threshold is 1.0 (default/no-merge) but we have a target r,
                    # we set specified_threshold to None to allow bipartite_soft_matching to use r.
                    if torch.all(self.threshold == 1.0) and r > 0:
