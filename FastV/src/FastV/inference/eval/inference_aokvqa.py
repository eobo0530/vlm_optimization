...
dataset = load_from_disk("/home/aips/FastV/data/aokvqa/validation")
...
if __name__=="__main__":
    ...
    parser.add_argument('--load-4bit', action='store_true', help='whether to use 4bit quantization')
    parser.add_argument('--load-8bit', action='store_true', help='whether to use 8bit quantization')
    parser.add_argument('--limit-samples', type=int, default=None, help='limit number of samples for quick benchmark')
    ...
    class InferenceArgs:
        ...
        temperature = 0.2
        max_new_tokens = 512
        load_8bit = pargs.load_8bit
        load_4bit = pargs.load_4bit
        debug = False
    ...
    print("Forcing model to GPU and FP16...")
    ...
    if hasattr(model.model, 'reset_fastv'):
        ...
        print("[WARNING] model.model does not have 'reset_fastv' attribute. Skipping reset.")
    ...
    def inference(prompts,images):
        ...
        for prompt,image in tqdm(zip(prompts,images),total=len(prompts)):
            ...
            t1 = time.time()
            ttft = t1 - t0
            if not hasattr(inference, "ttfts"):
                inference.ttfts = []
            inference.ttfts.append(ttft)
        ...
        avg_ttft = sum(inference.ttfts) / len(inference.ttfts)
        peak_vram = torch.cuda.max_memory_allocated() / (1024 ** 3)
        print(f"\n[BENCHMARK] Avg TTFT: {avg_ttft:.4f} s, Peak VRAM: {peak_vram:.2f} GB")
