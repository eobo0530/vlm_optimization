            ...
            if self.gradient_checkpointing and self.training:
                ...
                if USE_FAST_V and FASTV_INPLACE:
                    ...
                    if idx<AGG_LAYER:
                        ...
                        last_layer_attention_avg = torch.mean(last_layer_attention, dim=1, keepdim=False)[0]
                        ...
                        if last_layer_attention_avg_last_tok_image.size(0) > 0:
                            ...
                            keep_indexs = torch.arange(seq_length_with_past, device=device)
                ...
                # FastV Token Rerank, Attention Mask Implementation
                    ...
                    if idx<AGG_LAYER:
                        ...
                        if idx!=0:
                            last_layer_attention = layer_outputs[1]
                            # compute average attention (Batch-1 assumed for accurate per-sample pruning)
                            last_layer_attention_avg = torch.mean(last_layer_attention, dim=1, keepdim=False)[0]
                            ...
                            if last_layer_attention_avg_last_tok_image.size(0) > 0:
                                ...
                                # Safety: ensure top_attention_rank_index is on correct device and dtype
                                top_attention_rank_index = top_attention_rank_index.to(device=gen_attention_mask.device, dtype=torch.long)
                                
                                gen_attention_mask[:, image_start:image_end] = False
                                
                                # Robust indexing: ensure indices are within [0, gen_attention_mask.size(1))
                                mask_len = gen_attention_mask.size(1)
                                valid_mask = (top_attention_rank_index >= 0) & (top_attention_rank_index < mask_len)
                                valid_indices = top_attention_rank_index[valid_mask]
                                
                                if valid_indices.numel() > 0:
                                    # Use scatter_ for more robust indexing on GPU
                                    gen_attention_mask.scatter_(1, valid_indices.unsqueeze(0).expand(batch_size, -1), True)
